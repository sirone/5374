...
/*   var descriptions = new Array(); */
  ...
  function csvToArray(filename, cb) {
    $.get(filename, function(csvdata) {
      ...
      csvdata = csvdata.replace(/(,[亜-熙ぁ-んァ-ヶＡ-Ｚ\w\#\/]*?)<br \/>/gm, "$1\n" );
      console.log(csvdata);
  ...
  function updateAreaList() {
    csvToArray("data/area_days.csv?v="+(+new Date()), function(tmp) {
      ...
      csvToArray("data/center.csv?v="+(+new Date()), function(tmp) {
        //ゴミ処理センターのデータを解析します。
        //表示上は現れませんが、
  ...
  function createMenuList(after_action) {
    // 備考データを読み込む
    csvToArray("data/remarks.csv?v="+(+new Date()), function(data) {
      data.shift();
      for (var i in data) {
        remarks.push(new RemarkModel(data[i]));
      }
    });
    csvToArray("data/description.csv?v="+(+new Date()), function(data) {
      data.shift();
      for (var i in data) {
        descriptions.push(new DescriptionModel(data[i]));
      }

      csvToArray("data/target.csv?v="+(+new Date()), function(data) {
    ...
    for (var i in areaModel.trash) {
	  ...
	  if (trash.mostRecent === undefined) {
	    leftDayText == "不明";
          ...
          if (ableSVG && SVGLabel) {
            accordionHTML += '<img src="' + description.styles + '?v=' + (+new Date()) + '" alt="' + description.label + '"  />';
          } else {
